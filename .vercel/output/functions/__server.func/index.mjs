...
*/
var DEFAULT_APP_NAME = "Sonara Music";
...
function appNameFromHost(hostHeader) {
	...
	return slug.split("-").filter(Boolean).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ") || "Sonara Music";
}
...
function grokPwaHeadTags(appName = DEFAULT_APP_NAME) {
	return [
		...
		["theme-color", "<meta name=\"theme-color\" content=\"#030303\">"]
	];
}
...
function resolveOgTitle(site = {}, appName = DEFAULT_APP_NAME, host = "", documentTitle = "") {
	...
	if (fromHost && fromHost !== "Sonara Music") return fromHost;
	return String(appName ?? "").trim() || "Sonara Music";
}
...
function normalizeHeadContext(ctx = {}) {
	...
	return {
		appName: resolveOgTitle(site, ctx.appName ?? "Sonara Music", ctx.host ?? ""),
...
function injectGrokPwaHead(html, ctx = {}) {
	...
	const appName = resolveOgTitle(site, ctx.appName ?? "Sonara Music", host, documentTitle);
...
var _lazy_OAjLvC = defineLazyEventHandler(() => import("./_chunks/ssr-renderer.mjs"));
var findRoute = /* @__PURE__ */ (() => {
	const data = {
		route: "/**",
		handler: _lazy_OAjLvC
	};
	return ((_m, p) => {
		return {
			data,
